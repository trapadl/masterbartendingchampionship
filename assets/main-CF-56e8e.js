import{s as g,l as Q,g as K,a as W,h as J,i as X,b as Z}from"./countdown-Br4p3XrO.js";import{r as p,R as a,c as D,O as ee,a as te,b as R}from"./OrganizerSelect-DBFCZIjq.js";class ae{async fetchLeaderboardData(){try{const{data:t,error:r}=await g.from("bartenders").select(`
          id, 
          full_name,
          rating,
          rating_rd,
...
`;function ge(e){if(!e)return;const t=e.closest(".profile-modal");t&&(e.addEventListener("click",r=>{r.preventDefault(),r.stopPropagation(),B(t)}),t.addEventListener("click",r=>{r.target===t&&B(t)}),document.addEventListener("keydown",r=>{r.key==="Escape"&&t.style.display==="flex"&&B(t)}))}function B(e){e&&(e.style.opacity="0",setTimeout(()=>{e.style.display="none",e.style.opacity="1";const t=document.getElementById(e.dataset.previousFocus);t&&t.focus(),e.dispatchEvent(new CustomEvent("modalclose"))},300))}function pe(e){var r;if(!e)return;e.dataset.previousFocus=((r=document.activeElement)==null?void 0:r.id)||"",e.style.display="flex",e.style.opacity="0",e.offsetHeight,requestAnimationFrame(()=>{e.style.opacity="1"});const t=e.querySelector('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])');t&&t.focus()}const fe={initializeFocusTrap(e){const t=e.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'),r=t[0],n=t[t.length-1];e.addEventListener("keydown",o=>{o.key==="Tab"&&(o.shiftKey?document.activeElement===r&&(o.preventDefault(),n.focus()):document.activeElement===n&&(o.preventDefault(),r.focus()))})},show:pe,close:B};window.modalHelpers=fe;let I=!1;function be(){return`
  ...
  `}async function he(){if(I)return;const e=document.querySelector(".modal-wrapper");if(!e){console.warn("Modal wrapper not found, skipping initialization");return}const t=e.querySelectorAll(".modal-close");t.length>0&&t.forEach(r=>ge(r)),window.openProfile=(r,n)=>{we(document.getElementById("profile-modal"),r,n)},document.querySelectorAll(".action-button").forEach(r=>{r.addEventListener("click",async n=>{var E,l;n.preventDefault();const o=`${r.dataset.modal}-modal`,i=document.getElementById(o);if(!i){console.warn(`Modal ${o} not found`);return}const c=i.querySelector(`#${r.dataset.modal}-content`);if(!c){console.warn(`Content div not found in modal ${o}`);return}let m;try{m=await g.auth.getSession()}catch(b){if((E=b.message)!=null&&E.includes("storage is not allowed")){const{data:h}=await g.auth.getUser();m={data:{session:h?{user:h}:null}}}else throw b}!!((l=m==null?void 0:m.data)!=null&&l.session)?(c._root||(c._root=R(c)),r.dataset.modal==="claim-profile"?ye(c):r.dataset.modal==="submit-result"&&c._root.render(a.createElement(me,{onSuccess:()=>{i.style.display="none"},onError:b=>{console.error("Error submitting result:",b)}}))):c.innerHTML=oe(),i.style.display="flex",i.focus()})}),I=!0}async function ye(e){try{const{data:{user:t}}=await g.auth.getUser();if(!t)throw new Error("Authentication required");const{data:r}=await g.from("submissions").select("id, status").eq("user_id",t.id).eq("status","pending").like("content",'%"type":"profile_claim"%'),n=r==null?void 0:r[0];n?(e.innerHTML=ie(n),ve()):(e._root||(e._root=R(e)),e._root.render(a.createElement(se,{onSuccess:()=>{const o=e.closest(".profile-modal");o&&(o.style.display="none")},onError:o=>{console.error("Error submitting claim:",o)}})))}catch(t){console.error("Error handling claim modal:",t),e.innerHTML=`
      <div class="error-message">
        An error occurred. Please try again later.
      </div>
    `}}function ve(){const e=document.getElementById("cancel-claim");e&&e.addEventListener("click",async()=>{try{const{data:{user:t}}=await g.auth.getUser();if(!t)return;const{data:r}=await g.from("submissions").select("*").eq("user_id",t.id).eq("status","pending").single();if(r){const{error:n}=await g.from("submissions").update({status:"cancelled"}).eq("id",r.id);if(n)throw n;const o=document.getElementById("claim-profile-modal");o&&(o.style.display="none");const i=document.createElement("div");i.className="success-message mb-4 p-3 bg-green-500/20 border border-green-500 rounded text-green-200",i.textContent="Claim cancelled successfully",document.body.appendChild(i),setTimeout(()=>{i.remove(),location.reload()},3e3)}}catch(t){console.error("Error cancelling claim:",t),alert(t.message)}})}const O={initializeFocusTrap(e){const t=e.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'),r=t[0],n=t[t.length-1];e.addEventListener("keydown",o=>{o.key==="Tab"&&(o.shiftKey?document.activeElement===r&&(o.preventDefault(),n.focus()):document.activeElement===n&&(o.preventDefault(),r.focus()))})},show(e){var r;if(!e)return;e.dataset.previousFocus=((r=document.activeElement)==null?void 0:r.id)||"",e.style.display="flex",e.style.opacity="0",e.offsetHeight,requestAnimationFrame(()=>{e.style.opacity="1"});const t=e.querySelector('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])');t&&t.focus()},close(e){e&&(e.style.opacity="0",setTimeout(()=>{e.style.display="none",e.style.opacity="1";const t=document.getElementById(e.dataset.previousFocus);t&&t.focus(),e.dispatchEvent(new CustomEvent("modalclose"))},300))}};window.modalHelpers=O;function we(e,t,r){var o,i;const n=(i=(o=window.leaderboardData)==null?void 0:o[t])==null?void 0:i[r];n&&(document.getElementById("modal-name").textContent=n.name,document.getElementById("modal-points").textContent=n.points,document.getElementById("modal-bio").textContent=n.bio||"No bio available",O.show(e))}let T={traditional:[],flair:[],speed:[],legacy:[]};async function Ee(e){if(!e)return!1;const{data:t}=await g.from("bartenders").select("id").contains("users_attempting_to_claim",[e]);return(t==null?void 0:t.length)>0}async function _e(e){if(!e)return!1;const{data:t}=await g.from("bartenders").select("is_profile_claimed").eq("user_id",e).single();return(t==null?void 0:t.is_profile_claimed)||!1}async function xe(){const e=document.querySelector(".column-right-content");if(e)try{await Ce();const{data:{user:t}}=await g.auth.getUser(),r=t==null?void 0:t.id,n=await _e(r),o=await Ee(r),i=!n&&!o;e.innerHTML=await Ne(i,o);const c=document.createElement("div");c.innerHTML=be(),document.body.appendChild(c),await ke(),await he()}catch(t){console.error("Error initializing leaderboards:",t),e.innerHTML='<div class="error-message">Error loading leaderboard data</div>'}}async function Ce(){try{T=await re.fetchLeaderboardData()}catch(e){throw console.error("Error loading leaderboard data:",e),e}}async function Ne(e,t){return`
    ${Se(e,t)}
    ${F("speed","MBC Speed Leaderboard")}
    ${F("traditional","MBC Traditional Leaderboard",!0)}
    ${F("flair","MBC Flair Leaderboard",!0)}
  `}function Se(e,t){let r="";return t?r='<button class="action-button" disabled>Claim Pending</button>':e&&(r='<button class="action-button" data-modal="claim-profile">Claim Profile</button>'),`
  ...
  `,r.style.display="flex",Le(e,t)};async function Le(e,t){try{const r=await window.supabase.from("legacy_results").select(`
        placement,
  ...
  `}window.getBaseUrl=K;window.getAbsolutePath=W;document.addEventListener("DOMContentLoaded",function(){j(),J(),X(),xe(),Z()});window.addEventListener("load",j);
