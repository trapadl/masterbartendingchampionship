import{s as p,l as Z,g as ee,a as te,h as ae,i as re,b as ne}from"./countdown-C5GiBqwj.js";import{r as f,R as r,c as O,O as oe,a as ie,b as j}from"./OrganizerSelect-BZRqFvPl.js";class se{async fetchLeaderboardData(){try{const{data:t,error:a}=await p.from("legacy_results").select(`
          id,
...
`;function ye(e){if(!e)return;const t=e.closest(".profile-modal");t&&(e.addEventListener("click",a=>{a.preventDefault(),a.stopPropagation(),M(t)}),t.addEventListener("click",a=>{a.target===t&&M(t)}),document.addEventListener("keydown",a=>{a.key==="Escape"&&t.style.display==="flex"&&M(t)}))}function M(e){e&&(e.style.opacity="0",setTimeout(()=>{e.style.display="none",e.style.opacity="1";const t=document.getElementById(e.dataset.previousFocus);t&&t.focus(),e.dispatchEvent(new CustomEvent("modalclose"))},300))}function we(e){var a;if(!e)return;e.dataset.previousFocus=((a=document.activeElement)==null?void 0:a.id)||"",e.style.display="flex",e.style.opacity="0",e.offsetHeight,requestAnimationFrame(()=>{e.style.opacity="1"});const t=e.querySelector('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])');t&&t.focus()}const ve={initializeFocusTrap(e){const t=e.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'),a=t[0],i=t[t.length-1];e.addEventListener("keydown",s=>{s.key==="Tab"&&(s.shiftKey?document.activeElement===a&&(s.preventDefault(),i.focus()):document.activeElement===i&&(s.preventDefault(),a.focus()))})},show:we,close:M};window.modalHelpers=ve;let U=!1;function Ee(){return`
  ...
  `}async function _e(){if(U)return;const e=document.querySelector(".modal-wrapper");if(!e){console.warn("Modal wrapper not found, skipping initialization");return}const t=e.querySelectorAll(".modal-close");t.length>0&&t.forEach(a=>ye(a)),window.openProfile=(a,i)=>{Ne(document.getElementById("profile-modal"),a,i)},document.querySelectorAll(".action-button").forEach(a=>{a.addEventListener("click",async i=>{var _,c;i.preventDefault();const s=`${a.dataset.modal}-modal`,o=document.getElementById(s);if(!o){console.warn(`Modal ${s} not found`);return}const n=o.querySelector(`#${a.dataset.modal}-content`);if(!n){console.warn(`Content div not found in modal ${s}`);return}let d;try{d=await p.auth.getSession()}catch(g){if((_=g.message)!=null&&_.includes("storage is not allowed")){const{data:h}=await p.auth.getUser();d={data:{session:h?{user:h}:null}}}else throw g}!!((c=d==null?void 0:d.data)!=null&&c.session)?(n._root||(n._root=j(n)),a.dataset.modal==="claim-profile"?xe(n):a.dataset.modal==="submit-result"&&n._root.render(r.createElement(be,{onSuccess:()=>{o.style.display="none"},onError:g=>{console.error("Error submitting result:",g)}}))):n.innerHTML=de(),o.style.display="flex",o.focus()})}),U=!0}async function xe(e){try{const{data:{user:t}}=await p.auth.getUser();if(!t)throw new Error("Authentication required");const{data:a}=await p.from("submissions").select("id, status").eq("user_id",t.id).eq("status","pending").like("content",'%"type":"profile_claim"%'),i=a==null?void 0:a[0];i?(e.innerHTML=me(i),Ce()):(e._root||(e._root=j(e)),e._root.render(r.createElement(ue,{onSuccess:()=>{const s=e.closest(".profile-modal");s&&(s.style.display="none")},onError:s=>{console.error("Error submitting claim:",s)}})))}catch(t){console.error("Error handling claim modal:",t),e.innerHTML=`
      <div class="error-message">
        An error occurred. Please try again later.
      </div>
    `}}function Ce(){const e=document.getElementById("cancel-claim");e&&e.addEventListener("click",async()=>{try{const{data:{user:t}}=await p.auth.getUser();if(!t)return;const{data:a}=await p.from("submissions").select("*").eq("user_id",t.id).eq("status","pending").single();if(a){const{error:i}=await p.from("submissions").update({status:"cancelled"}).eq("id",a.id);if(i)throw i;const s=document.getElementById("claim-profile-modal");s&&(s.style.display="none");const o=document.createElement("div");o.className="success-message mb-4 p-3 bg-green-500/20 border border-green-500 rounded text-green-200",o.textContent="Claim cancelled successfully",document.body.appendChild(o),setTimeout(()=>{o.remove(),location.reload()},3e3)}}catch(t){console.error("Error cancelling claim:",t),alert(t.message)}})}const Y={initializeFocusTrap(e){const t=e.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'),a=t[0],i=t[t.length-1];e.addEventListener("keydown",s=>{s.key==="Tab"&&(s.shiftKey?document.activeElement===a&&(s.preventDefault(),i.focus()):document.activeElement===i&&(s.preventDefault(),a.focus()))})},show(e){var a;if(!e)return;e.dataset.previousFocus=((a=document.activeElement)==null?void 0:a.id)||"",e.style.display="flex",e.style.opacity="0",e.offsetHeight,requestAnimationFrame(()=>{e.style.opacity="1"});const t=e.querySelector('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])');t&&t.focus()},close(e){e&&(e.style.opacity="0",setTimeout(()=>{e.style.display="none",e.style.opacity="1";const t=document.getElementById(e.dataset.previousFocus);t&&t.focus(),e.dispatchEvent(new CustomEvent("modalclose"))},300))}};window.modalHelpers=Y;function Ne(e,t,a){var s,o;const i=(o=(s=window.leaderboardData)==null?void 0:s[t])==null?void 0:o[a];i&&(document.getElementById("modal-name").textContent=i.name,document.getElementById("modal-points").textContent=i.points,document.getElementById("modal-bio").textContent=i.bio||"No bio available",Y.show(e))}let T={traditional:[],flair:[],speed:[],legacy:[]},I={traditional:0,flair:0,speed:0,legacy:0},P={traditional:"total",flair:"total",speed:"total",legacy:"total"};async function Se(e){if(!e)return!1;const{data:t}=await p.from("bartenders").select("id").contains("users_attempting_to_claim",[e]);return(t==null?void 0:t.length)>0}async function ke(e){if(!e)return!1;const{data:t}=await p.from("bartenders").select("is_profile_claimed").eq("user_id",e).single();return(t==null?void 0:t.is_profile_claimed)||!1}async function Le(){const e=document.querySelector(".column-right-content");if(e)try{await Pe();const{data:{user:t}}=await p.auth.getUser(),a=t==null?void 0:t.id,i=await ke(a),s=await Se(a),o=!i&&!s;e.innerHTML=await $e(o,s);const n=document.createElement("div");n.innerHTML=Ee(),document.body.appendChild(n),await Be(),await _e(),window.loadNextPage=Q,window.loadSpecificPage=D}catch(t){console.error("Error initializing leaderboards:",t),e.innerHTML='<div class="error-message">Error loading leaderboard data</div>'}}async function Pe(){try{T=await le.fetchLeaderboardData()}catch(e){throw console.error("Error loading leaderboard data:",e),e}}async function $e(e,t){return`
    ${qe(e,t)}
  ...
  `}function qe(e,t){let a="";return t?a='<button class="action-button" disabled>Claim Pending</button>':e&&(a='<button class="action-button" data-modal="claim-profile">Claim Profile</button>'),`
    <div id="legacy-section" class="leaderboard-container">
      ...
      <div class="leaderboard-content">
        ${z({data:T.legacy,type:"legacy",page:I.legacy,filter:P.legacy})}
      </div>
    </div>
  ...
  `,a.style.display="flex",Me(e,t)};async function Me(e,t){try{const a=await window.supabase.from("legacy_results").select(`
        placement,
  ...
  `}window.getBaseUrl=ee;window.getAbsolutePath=te;document.addEventListener("DOMContentLoaded",function(){K(),ae(),re(),Le(),ne()});window.addEventListener("load",K);
